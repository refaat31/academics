...
void showStatus()
{
    ...
    for (int k = 0; k < 5; k++)
    {
        ...
        if (flag[k] != 0)
        {
            if (currentlyAtTable[k] == 0)
                printf("Philosopher %d is not at the table \t", k + 1);
            else
...
{
    sem_wait(&philAtTable);
    currentlyAtTable[phnum] = 1;
    sem_wait(&mutex);
...
void put_fork(int phnum)
{
    ...
    sem_post(&mutex);
    sem_post(&philAtTable);
    currentlyAtTable[phnum] = 0;
}
